		...
		switch(option){
			...
			case 'D':
				...
				if((offset + length) >= CPU_MEM)
					printf("\nThe length and offset is out of boundary.\n");
				else
			...
			case 'm':
			case 'M':
				printf("\n\nEnter the  address to modify(in HEX): ");
...
//This functions handles the flag
...
//It checks the condition code and flags to determine if branch is needed
int checkBranch(){
